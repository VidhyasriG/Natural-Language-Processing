...
tf.flags.DEFINE_float("l2_reg_lambda", 100, "L2 regularization lambda (default: 0.0)")
tf.flags.DEFINE_integer("min_frequency", 2, "filter vocab based on minimum frequency")
...
tf.flags.DEFINE_integer("num_checkpoints", 40, "Number of checkpoints to store (default: 5)")
# Misc Parameters
...
if FLAGS.enable_word_embeddings and cfg['word_embeddings']['default'] is not None:
    ...
    if not embedding_name == 'both':
        embedding_dimension = cfg['word_embeddings'][embedding_name]['dimension']
    else:
        embedding_dimension = cfg['word_embeddings']['word2vec']['dimension'] + cfg['word_embeddings']['glove200']['dimension']
else:
...
if dataset_name == "mrpolarity":
    ...
    datasets = data_helpers.get_datasets_semeval(sys.argv[1])
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        if FLAGS.enable_word_embeddings and cfg['word_embeddings']['default'] is not None:
            ...
            initW = None
            if embedding_name == 'both':
                print("Load word2vec file {}".format(cfg['word_embeddings']['word2vec']['path']))
                initW_google = data_helpers.load_embedding_vectors_word2vec(vocabulary,
                                                                     cfg['word_embeddings']['word2vec']['path'],
                                                                     cfg['word_embeddings']['word2vec']['binary'])
                print("word2vec file has been loaded")

                print("Load glove file {}".format(cfg['word_embeddings']['glove200']['path']))
                initW_glove = data_helpers.load_embedding_vectors_glove(vocabulary,
                                                                  cfg['word_embeddings']['glove200']['path'],
                                                                  int(cfg['word_embeddings']['glove200']['dimension']))
                print("glove file has been loaded\n")
                initW = np.concatenate((initW_google,initW_glove), axis=1)
                # print(initW.shape)
                # sys.exit()
            if embedding_name == 'word2vec':
                ...
                initW = data_helpers.load_embedding_vectors_glove(vocabulary,
                                                                  ...
                                                                  int(cfg['word_embeddings']['glove']['dimension']))
                print("glove file has been loaded\n")
                ...
                # load embedding vectors from the glove
                print("Load glove file {}".format(cfg['word_embeddings']['glove300']['path']))
                initW = data_helpers.load_embedding_vectors_glove(vocabulary,
                                                                  cfg['word_embeddings']['glove300']['path'],
                                                                  int(cfg['word_embeddings']['glove300']['dimension']))
                print("glove file has been loaded\n")
        ...
        for batch in batches:
            ...
            if current_step % FLAGS.evaluate_every == 0:
                print("\nEvaluation:")
                accuracy, step = dev_step(x_dev, y_dev, writer=dev_summary_writer)
                print("")
            if current_step % FLAGS.checkpoint_every == 0:
                path = saver.save(sess, checkpoint_prefix, global_step=current_step)
                print("Saved model checkpoint to {}\n".format(path))
                # when validation accuracy reaches 65% after 100 steps (100 batches) stop and used that saved model
                # for testing
                if accuracy > 0.65 and step > 100:
