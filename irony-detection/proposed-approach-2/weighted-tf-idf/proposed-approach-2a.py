...
y_true = data_test["label"].tolist()\for i in range(len(data)):
    word_vocab.extend(word_tokenize(data.loc[i, "text"]))
word_vocab=list(set(word_vocab))
multiplication_vectorizer=TfidfVectorizer(vocabulary=word_vocab)
multiplication_vectorizer.fit(str(data.loc[i, "text"]) for i in range(len(data)))
if word_embedding_type == "glove":
	x_pred = get_glove_vectors(data_test, model, vect_size,vectorizer=multiplication_vectorizer)
else:
